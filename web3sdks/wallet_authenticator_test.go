...
func TestRejectIncorrectDomain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	_, err := sdk.Auth.Verify("test.web3sdks.com", payload, nil)

	assert.Equal(t, err.Error(), "Expected domain 'test.web3sdks.com' does not match domain on payload 'web3sdks.com'")
}
...
func TestRejectTokenIncorrectDoain(t *testing.T) {
	...
	sdk.Auth.UpdatePrivateKey(secondaryPrivateKey)
	token, _ := sdk.Auth.GenerateAuthToken(domain, payload, nil)
	_, err := sdk.Auth.Authenticate("test.web3sdks.com", token)

	assert.Equal(t, err.Error(), "Expected token to be for the domain 'test.web3sdks.com', but found token with domain 'web3sdks.com'")
}
